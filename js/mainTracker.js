(function (w, d, $) {
    if (!$) { conole.log('jquery not initialized'); return !1 }
    ...
    function getCookie(name) {
        var str, matches = d.cookie.match(new RegExp("(?:^|; )" + name.replace(/([\.$?*|{}\(\)\[\]\\\/\+^])/g, '\\$1') + "=([^;]*)")); try { str = decodeURIComponent(matches[1]) } catch (e) { str = undefined }
    ...
    var userid = getCookie('tildauid'); var sessionid = getCookie('tildasid'); var isSendScrollEvent = { p10: 0, p25: 0, p50: 0, p75: 0, p90: 0 }; var dStartReadTime = new Date(); var isWindowActive = !0; w.onVisibilityWindowChange = function (callback) {
        var visible = !0; if (!callback) { throw new Error('no callback given') }
        ...
        var top = $(document).scrollTop(); var wh = $(window).height(); var dh = $(document).height(); var hh = 0, fh = 0; var p; var d = new Date(); isWindowActive = !0; if ($('#t-header').length > 0) { hh = $('#t-header').height() }
        if ($('#t-footer').length > 0) { fh = $('#t-footer').height() }
        p = parseInt((top - hh + wh) * 100 / (dh - hh - fh)); if (p < 10) { return }
        if (p >= 10 && !isSendScrollEvent.p10) { tildaParams.page = '/tilda/scroll/10/'; w.tildastat('pageview'); isSendScrollEvent.p10 = !0 } else {
            if (p >= 24) { if (isSendScrollEvent.p25 == 0) { tildaParams.page = '/tilda/scroll/25/'; w.tildastat('pageview'); isSendScrollEvent.p25 = w.setTimeout(function () { w.clearTimeout(isSendScrollEvent.p25); isSendScrollEvent.p25 = -1 }, 5000); return } else { if (p < 51 && isSendScrollEvent.p25 == -1) { isSendScrollEvent.p25 = 0; return } } }
            ...
            if (p >= 89) { if (isSendScrollEvent.p90 == 0) { tildaParams.page = '/tilda/scroll/90/'; w.tildastat('pageview'); isSendScrollEvent.p90 = w.setTimeout(function () { w.clearTimeout(isSendScrollEvent.p90); isSendScrollEvent.p90 = -1 }, 5000); return } else { if (isSendScrollEvent.p90 == -1) { isSendScrollEvent.p90 = 0; return } } }
        }
    }; w.tildastat = function (event, params) {
        var staturl = 'https:' + '//stat' + '.tildacdn.' + 'com/event/'; if (!event) { return !1 }
        if (params) { $.extend(tildaParams, params) }
        if (!userid) { userid = generateUniqID() }
        if (!sessionid) { sessionid = generateUniqID() }
        switch (event) {
            case 'create': var tmp = w.location.hostname; if (tmp.substring(0, 4) == 'www.') { tmp = tmp.substring(4) }
                ...
                break; case 'pageview': if (!tildaParams.page) {
                    console.log('TildaStat: page empty'); var tmp = w.location.hostname; if (tmp.substring(0, 4) == 'www.') { tmp = tmp.substring(4) }
                    try { if (tmp.lastIndexOf('.') == tmp.length - 1) { tmp = tmp.slice(0, -1) } } catch (e) { }
                ...
                tildaParams.page = ''; w.tildastatload = !0; break; case 'readtime': try {
                    if (w.tildastatscroll == 'yes') {
                        if (isWindowActive) { tildaParams.page = '/tilda/readtime/'; isWindowActive = !1; w.tildastat('pageview') }
                        w.setTimeout(function () { w.tildastat('readtime') }, 15000)
                    }
                } catch (e) { }
                break; case 'scroll': try { if (w.tildastatscroll == 'yes') { $(w).on('scroll', t_throttle(w.tildastat_scrollEvent, 500)) } } catch (e) { }
                break; default: break
        }
    }; w.tildastat('create'); w.setTimeout(function () { w.tildastat('pageview'); w.tildastat('readtime'); w.tildastat('scroll') }, 2000)
